...
class Compiler extends Tapable {
	...
	run(callback) {
		...
		const onCompiled = (err, compilation) => {
			...
			debugger // 所有需要输出的文件已经生成好，询问插件哪些文件需要输出，哪些不需要。
			...
			debugger // 确定好要输出哪些文件后，执行文件输出，可以在这里获取和修改输出内容。
		...
		this.hooks.beforeRun.callAsync(this, err => {
			...
			this.hooks.run.callAsync(this, err => {
				...
				this.readRecords(err => {
					if (err) return finalCallback(err);
					debugger // 该事件是为了告诉插件一次新的编译将要启动，同时会给插件带上 compiler 对象。
					this.compile(onCompiled);
				});
			});
		});
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, err => {
			...
			debugger // 当 Webpack 以开发模式运行时，每当检测到文件变化，一次新的 Compilation 将被创建。
			//一个 Compilation 对象包含了当前的模块资源、编译生成资源、变化的文件等。
			//Compilation 对象也提供了很多事件回调供插件做扩展。
			const compilation = this.newCompilation(params);
			debugger //一个新的 Compilation 创建完毕，即将从 Entry 开始读取文件，根据文件类型和配置的 Loader 对文件进行编译，编译完后再找出该文件依赖的文件，递归的编译和解析。
